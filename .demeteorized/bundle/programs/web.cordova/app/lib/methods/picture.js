	...
	pictureCreator: function (picture, link) {
		var self = this;
 		if(!Match.test(picture, Match.Where(function(picture){
			...
			throw new Meteor.Error("wrong formatting picture", "picture should be StringB64File or FileInfoObject");
		}
		if(!Match.test(link, Object)){
			throw new Meteor.Error("wrong formatting linkObject");
		}
		if(!Match.test(link.collection, Match.Where(function(collection){
			...
			throw new Meteor.Error("wrong formatting link.collection", "link.collection must match with a collection name");
		}
		...
		if(this.isSimulation){
			Session.set("successMessage", "L'image est ajouter à ce travail" );
		}
